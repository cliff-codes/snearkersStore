...
const Showcase = () => {
  ...
  return (
    <Container sx={{overflow: "hidden", pb: "24px"}} >
        {
            isSmallScreen && <SearchBar/>
        }
        <Box display={'flex'}  mt={'48px'} width={'100%'} justifyContent={'space-between'} alignItems={'center'} >
            <Box display={'flex'} flexDirection={'column'} justifyContent={'space-between'} gap={'30px'}>
            ...
            <Box position={'relative'} >
                ...
                <Box position={'absolute'} top={isSmallScreen ? '12px' : '24px'} left={isSmallScreen ? '12px' : '24px'} width={isSmallScreen ? '200px' : isMediumScreen ? '300px' : '500px' } height={isSmallScreen ? '200px' : isMediumScreen ? '300px' : '500px'} borderRadius={'4px'} sx={{
